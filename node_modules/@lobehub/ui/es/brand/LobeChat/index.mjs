'use client';

import FlexBasic_default from "../../Flex/FlexBasic.mjs";
import LobeChatText_default from "../LobeChatText/index.mjs";
import { useStyles } from "../LobeHub/style.mjs";
import Logo3d_default from "../Logo3d/index.mjs";
import LogoFlat_default from "../LogoFlat/index.mjs";
import LogoMono_default from "../LogoMono/index.mjs";
import Divider_default from "../components/Divider.mjs";
import { memo } from "react";
import { Fragment as Fragment$1, jsx, jsxs } from "react/jsx-runtime";
import { useTheme } from "antd-style";

//#region src/brand/LobeChat/index.tsx
const LobeChat = memo(({ type = "3d", size = 32, style, extra, className, ...rest }) => {
	const theme = useTheme();
	const { styles } = useStyles();
	let logoComponent;
	switch (type) {
		case "3d":
			logoComponent = /* @__PURE__ */ jsx(Logo3d_default, {
				alt: "LobeChat",
				size,
				...rest
			});
			break;
		case "flat":
			logoComponent = /* @__PURE__ */ jsx(LogoFlat_default, {
				size,
				style
			});
			break;
		case "mono":
			logoComponent = /* @__PURE__ */ jsx(LogoMono_default, {
				size,
				style
			});
			break;
		case "text":
			logoComponent = /* @__PURE__ */ jsx(LobeChatText_default, {
				className,
				size,
				style,
				...rest
			});
			break;
		case "combine":
			logoComponent = /* @__PURE__ */ jsxs(Fragment$1, { children: [/* @__PURE__ */ jsx(Logo3d_default, {
				alt: "LobeChat",
				size
			}), /* @__PURE__ */ jsx(LobeChatText_default, {
				size,
				style: { marginLeft: Math.round(size / 4) }
			})] });
			if (!extra) logoComponent = /* @__PURE__ */ jsx(FlexBasic_default, {
				align: "center",
				className,
				flex: "none",
				horizontal: true,
				style,
				children: logoComponent
			});
			break;
	}
	if (!extra) return logoComponent;
	const extraSize = Math.round(size / 3 * 1.9);
	return /* @__PURE__ */ jsxs(FlexBasic_default, {
		align: "center",
		className,
		flex: "none",
		horizontal: true,
		style,
		...rest,
		children: [
			logoComponent,
			/* @__PURE__ */ jsx(Divider_default, {
				size: extraSize,
				style: { color: theme.colorFill }
			}),
			/* @__PURE__ */ jsx("div", {
				className: styles.extraTitle,
				style: { fontSize: extraSize },
				children: extra
			})
		]
	});
});
LobeChat.displayName = "LobeChatBrand";
var LobeChat_default = LobeChat;

//#endregion
export { LobeChat_default as default };
//# sourceMappingURL=index.mjs.map